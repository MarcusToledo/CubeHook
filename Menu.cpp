...
static LRESULT __stdcall wnd_proc(const HWND hwnd, const UINT u_msg, const WPARAM w_param, const LPARAM l_param) {
	...
	if (u_msg == WM_WINDOWPOSCHANGED) {
		...
		auto d3d9PresentParams = GMenu.getD3DPresentParameters();
		...
		MoveWindow(GMenu.getOverlayWindowHandle(), windowPositionStructure->x + windowInfo.cxWindowBorders, windowPositionStructure->y - windowInfo.cyWindowBorders, windowPositionStructure->cx, windowPositionStructure->cy, false);
	...
	if (u_msg == WM_KEYUP && w_param == VK_INSERT) {
		bool openedFlag = GMenu.IsMenuOpened();
	...
	if (GMenu.IsMenuOpened()) {
		...
		if (bWasMenuClicked) {
			if (u_msg == WM_KEYDOWN && bWasKeyPressed) {
				...
				return CallWindowProcA(reinterpret_cast<WNDPROC>(GMenu.getGameWindowProc()), hwnd, WM_KEYUP, w_param, l_param);
			}
